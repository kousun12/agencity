...
3. A valid proposal is evaluated by one separate sealed reviewer invocation against frozen, digest-addressed inputs: the proposal, current target, evidence, proposer relationship, product constitution, refinement policy, configured workspace charter and user constraints, target scope, runtime boundaries, model dispatch, and explicit reviewer limits. For refiner-produced proposals, the versioned input includes bounded redacted excerpts from the exact frozen trajectory snapshot plus its trigger, allowed kinds, review identity, and snapshot hash, so semantic grounding can be reviewed without loading repository-authored instructions. The reviewer receives 16,384 tokens, USD 1 cost, two runtime turn slots for one structured decision, and 120 seconds wall time; these limits are independent of unbounded parent defaults but never widen a smaller parent budget.
