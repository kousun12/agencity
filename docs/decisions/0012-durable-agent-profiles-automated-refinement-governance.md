...
This decision is implemented in the runtime and public product surfaces: durable profiles and prompt pins, scoped governed proposals, deterministic validation, a separate bounded sealed reviewer, application-time revalidation, storage-enforced atomic profile/non-skill application, staged tested-skill activation, terminal delivery, bounded reproposal chains, lineage-aware rollback evidence, exhaustive paged recovery, and exact rollback are present. Runtime, protocol, terminal-control, focused recovery, synchronization-divergence, export-audit, deletion-refusal, focused lifecycle, isolated linked-executable governance, and final deterministic aggregate evidence has passed. Workspace-charter and user-constraint configuration remains unavailable and is pinned as `null`; callers cannot select the reviewer. External live-provider, official Turso Sync, and Turso Cloud verification remains gated and unverified.
