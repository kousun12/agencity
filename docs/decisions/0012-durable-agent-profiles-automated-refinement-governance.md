...
3. A valid proposal is evaluated by one separate sealed reviewer invocation against frozen, digest-addressed inputs: the proposal, current target, evidence, proposer relationship, product constitution, refinement policy, configured workspace charter and user constraints, target scope, runtime boundaries, model dispatch, and explicit reviewer limits. The reviewer receives 16,384 tokens, USD 1 cost, two runtime turn slots for one structured decision, and 120 seconds wall time; these limits are independent of unbounded parent defaults but never widen a smaller parent budget.
...
This decision is implemented in the runtime and public product surfaces: durable profiles and prompt pins, scoped governed proposals, deterministic validation, a separate bounded sealed reviewer, application-time revalidation, storage-enforced atomic profile/non-skill application, staged tested-skill activation, terminal delivery, bounded reproposal chains, lineage-aware rollback evidence, exhaustive paged recovery, and exact rollback are present. Runtime, protocol, terminal-control, focused recovery, synchronization-divergence, export-audit, deletion-refusal, focused lifecycle, and isolated linked-executable governance evidence has passed. Final post-hardening aggregate verification remains pending. Workspace-charter and user-constraint configuration remains unavailable and is pinned as `null`; callers cannot select the reviewer. External live-provider, official Turso Sync, and Turso Cloud verification remains gated and unverified.
