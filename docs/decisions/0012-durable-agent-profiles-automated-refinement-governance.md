...
3. A valid proposal is evaluated by one separate sealed reviewer invocation against frozen, digest-addressed inputs: the proposal, current target, evidence, proposer relationship, product constitution, refinement policy, configured workspace charter and user constraints, target scope, runtime boundaries, model dispatch, and explicit reviewer limits. New freezes use input version 3. Every proposal receives deterministic canonical-JSON excerpts for all cited event payloads under one fixed 32 KiB aggregate budget, with canonical/redacted payload and excerpt digests, exact byte counts, truncation state, and credential/repository-instruction redaction provenance. Repository-authored instruction content and brokered secrets are excluded. Refiner-produced proposals additionally pin their trigger, allowed kinds, review identity, and source-snapshot hash. Retained version-1 and version-2 inputs remain readable. The reviewer receives 16,384 tokens, USD 1 cost, two runtime turn slots for one structured decision, and 120 seconds wall time; these limits are independent of unbounded parent defaults but never widen a smaller parent budget.
...
The sealed refiner selects artifacts by mechanism: memory for scoped retained facts and preferences, prompt notes for repeated behavioral tendencies, tested skills for reusable deterministic operations, and subagent specifications for recurring delegated roles. Repository-specific implementation and missing runtime primitives remain ordinary implementation work. If no allowed harness kind directly addresses the evidence, the review returns no change rather than substituting generic diligence or broadening the user's task.
...
Rejected proposals are terminal and immutable. A revised attempt is a new proposal that references the original rejection, changes substantive content, evidence, or evaluation intent, and remains bound to the same principal, origin/trigger, target kind, scope, and intended target. Automatic refinement permits at most one descendant for the original trigger-target chain; manual revisions remain subject to rate, concurrency, identity, and substantive-change bounds. The trajectory refiner's required objective evaluation is retained in the governed proposal's canonical identity, frozen reviewer input, inspection projection, and application history. It describes intended post-activation evaluation and is not an ordinary activation gate. Direct owner or agent proposals may omit it. Reviewer approval establishes charter compliance, not empirical improvement. Later attributable outcomes remain evaluation evidence and may support another proposal or rollback.

Generated skills must compile and pass their declared runtime tests before activation. This remains the only target-specific activation-time evaluation beyond ordinary deterministic revalidation; objective evaluation intent does not invoke a command or block activation. Reviewer approval cannot waive executable skill validation. Profile and single-target replacement rollback restores the exact content of an earlier approved version through a new immutable version and activation; modified content is a new proposal and requires review. An applied automatic local harness proposal also has one owner-only grouped inverse derived from its immutable edits: replacement creates an exact restoration version, creation deactivates the generated version, retirement reactivates the original version, and a multi-edit proposal commits every inverse atomically with one proposal-linked rollback event. Agent rollback evidence must be visible on the caller's exact branch lineage. Workspace-owner rollback may cite evidence from the same workspace.
...
- Recovery scans governed work oldest-first in bounded pages and continues until every nonterminal proposal and every terminal undelivered notice has been considered; no newest-only history cap may starve older work.
...
This decision is implemented in the runtime and public product surfaces: durable profiles and prompt pins, scoped governed proposals, deterministic validation, a separate bounded sealed reviewer, application-time revalidation, storage-enforced atomic profile/non-skill application, staged tested-skill activation, terminal delivery, bounded reproposal chains, lineage-aware rollback evidence, exhaustive paged recovery, exact single-target restoration, and proposal-level automatic create/replace/retire/multi-edit rollback are present. Runtime, protocol, terminal-control, focused recovery, synchronization-divergence, export-audit, deletion-refusal, focused lifecycle, and isolated linked-executable governance evidence has passed. Workspace-charter and user-constraint configuration remains unavailable and is pinned as `null`; callers cannot select the reviewer. External live-provider, official Turso Sync, and Turso Cloud verification remains gated and unverified.
