...
| Model loop and attributable context | **verified** | `test/integration/model-loop.test.ts` checks exact context/event provenance, response linkage, budgets, failures, recovery finalization. |
...
| Reactive event interface | **verified** | `test/integration/projection.test.ts` covers snapshot/catch-up races, duplicate notifications, disconnect/resume, stream abort, and session/branch isolation. `test/integration/protocol.test.ts` covers HTTP error/status mapping; SSE reconnect framing still lacks a transport-level test. |
...
|---|---:|---|
| Representative coding task with worker restarted after every committed cell and identical state | 1 | **verified** — `test/e2e/coding-task.test.ts` plans, reproduces a failing gate, edits with a digest precondition, verifies, checks fresh worker PIDs and projection equality after every cell, reopens storage, and confirms identical state without another model call. |
...
| Parent/child sessions detach/restart from durable task records | 2 | **deferred**. |
...
| Snapshot subscriber disconnect/resume misses no state; duplicates projection-neutral | 1 | **verified at core interface** — projection integration simulates duplicate at-least-once notifications, a snapshot/subscribe race, disconnect/resume, isolation, and async-stream abort. HTTP SSE transport itself lacks a focused integration test. |
