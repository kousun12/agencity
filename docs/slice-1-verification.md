...
| Autonomous model-to-TypeScript run | **verified/limited** | `AgentRunService` drives strict version-1 actions through disposable cells and distinct terminal states; `test/e2e/coding-task.test.ts` autonomously inspects, reproduces, edits, verifies, and returns a validated final with worker restart after every cell. Detached background execution and protocol-backed TUI remain later work. |
