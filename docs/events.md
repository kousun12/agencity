...
The provider-facing run input also derives a bounded `recentTrajectory` from canonical run actions and terminal cell outcomes. It retains at most eight recent completed actions and caps TypeScript source and result summaries independently. When the latest outcome is already present in the new exact-once observations, the trajectory references that observation instead of duplicating its result. This continuity projection does not alter the canonical observation ledger and lets each new model call reason about what it already attempted while the step instruction independently asks whether to call `finish` or execute one necessary `bun_console` action. Formal submissions remain internal. A successful `finish` publishes its exact assistant message only after required gates pass; failed or unknown gate evidence does not publish it.
