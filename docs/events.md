...
| `CellFailed` | `{ cellId, error: string, logs: string[], logStreams?: ("stdout" \| "stderr")[], durationMs: nonnegative number, causalEffectOutcomeEventIds?: string[], failure?: { code: "REPL_EPOCH_CHANGED", expected: ReplNamespaceStatus \| null, current: ReplNamespaceStatus, guidance: string } }` | Moves a running cell to failed; optional stream metadata follows the same alignment and compatibility rule, and no staged state/reference is appended. New failures include the exact `EffectOutcomeRecorded` event IDs for terminal failed, cancelled, or unknown effects whose convenience-helper error directly escaped the worker. Every ID must name an eligible effect requested by that cell. Omission remains readable for earlier schema-version-5 events; a present empty array explicitly means no causal effect was proven. `REPL_EPOCH_CHANGED` proves the submitted source was not evaluated because the attempt pin no longer matched worker-pool status; `expected: null` means a retained pre-feature action had no safe epoch pin. |
...
| `AgentRunModelAttemptStarted` | `{ runId, stepId, ordinal, attempt, contextId, callId, effectId, reason, providerInputVersion, providerInputDigest, estimatedInputTokens, contextWindow, replNamespace?: ReplNamespaceStatus, retryOfCallId? }` | Attributes the exact initial or provider-overflow model attempt to its retained provider candidate. New attempts pin the exact cold or warm named REPL namespace status included in provider input; omission remains readable for retained earlier schema-version-5 events. Only typed provider-confirmed overflow may create a later attempt over a strictly smaller candidate. |
...
| `GovernedRefinementProposed` | Immutable profile or harness proposal, proposer/origin, expected target, replacement, reason, predicted effect, evidence, optional post-activation objective evaluation intent, optional revised-proposal link, and stable fingerprint. New automatic-refiner admission requires the evaluation; direct owner or agent proposals may omit it. Retained schema-5 automatic proposals that predate this admission rule remain readable and synchronizable without an evaluation field. |
