...
| `schemaVersion` | positive integer | Payload/header version; exactly `6`. |
...
type ManagedProcessStatus = "queued" | "running" | EffectOutcome;
...
| `SessionTitleRequested` | `{ requestId, sourceMessageEventId, sourceMessageCursor, sourceMessageEventIds, mode, effectId?, modelDispatch?, providerInput?, estimatedInputTokens?, fallbackReason? }` | Retains one background automatic-title request over every eligible task-bearing input through an exact frontier. Eligible input is a direct client message, the session's initial delegated task, or parent-to-child family input; child results and sibling traffic are excluded. Model requests pin the auxiliary Luna dispatch and provider input. |
| `SessionTitleResolved` | `{ requestId, sourceMessageEventId, sourceMessageCursor, sourceMessageEventIds, sourceBranchId, method, title, verb, subject, intentSummary, sourceOutcomeEventId?, usage?, warnings?, usageSource?, fallbackReason? }` | Retains the bounded structured result and applies it only when automatic naming is active and its source frontier is newer than the currently applied title. Late results cannot replace a newer title. |
| `SessionTitleModeChanged` | `{ mode: "automatic", reason }` | Re-enables automatic title application after an explicit `SessionNamed` event selected manual naming. |
...
| `CellFailed` | `{ cellId, error: string, logs: string[], logStreams?: ("stdout" \| "stderr")[], durationMs: nonnegative number, causalEffectOutcomeEventIds?: string[], failure?: { code: "REPL_EPOCH_CHANGED", expected: ReplNamespaceStatus \| null, current: ReplNamespaceStatus, guidance: string } }` | Moves a running cell to failed; optional stream metadata must align with `logs`, and no staged state/reference is appended. New failures include the exact `EffectOutcomeRecorded` event IDs for terminal failed, cancelled, or unknown effects whose convenience-helper error directly escaped the worker. Every ID must name an eligible effect requested by that cell. A present empty causal list explicitly means no causal effect was proven. `REPL_EPOCH_CHANGED` proves the submitted source was not evaluated because the attempt pin no longer matched worker-pool status. |
...
| `ManagedProcessRegistered` | `{ processId, effectId, workspaceId, runId?, cellId, command, cwd?, identityToken, requestedAt }` | Atomically pairs a queued managed-process lifecycle with its non-idempotent outbox request before spawn. The retained owner route and random identity token are canonical; the console handle omits the token. |
| `ManagedProcessStarted` | `{ processId, effectId, identityToken, pid, processGroupId, startedAt }` | Moves the exact queued process to running after `EffectAttemptStarted`. The token and process group must match registration. PID alone is never recovery authority. The paired `EffectOutcomeRecorded` supplies the terminal succeeded, failed, cancelled, or unknown process state and bounded output. |
| `ManagedProcessStopFailed` | `{ processId, effectId, attempt, reason, error, processGroupIds, survivingProcessGroupIds, attemptedAt }` | Retains each failed TERM/KILL cleanup attempt while the process effect remains running. `processGroupIds` is the exact attempted set and `survivingProcessGroupIds` is the executing or unconfirmed subset after every attempted group has been processed. Inspection exposes the latest attempt and diagnostics; a later effect outcome remains the authoritative terminal process result. |
...
| `ModelCallRequested` | `{ callId, contextId, effectId, modelDispatch, providerInput, estimatedInputTokens, promptProvenance, attempt?, retryOfCallId?, contextWindow? }` | Links a logical model call or attributed overflow retry to exact context, immutable `agencity.model-dispatch.v2`, complete `agencity.provider-input.v2` candidate, estimate, capacity provenance, durable effect, and invocation prompt provenance. The candidate contains the exact provider-neutral text/tool-call/tool-result messages, fixed tools/order, cache contract, policies, token-relevant options, and provenance consumed by estimation, execution, and recovery. |
...
| `BudgetDebited` | `{ callId, tokens, costUsd, turns, wallTimeMs, usageSource }` (all nonnegative) | Adds provider-reported or conservative guard-estimate usage to projected counters. Provider cache read/write diagnostics do not change the debit; reported model usage debits `inputTokens + outputTokens`. |
...
| `AgentRunModelAttemptStarted` | `{ runId, stepId, ordinal, attempt, contextId, callId, effectId, reason, providerInputVersion, providerInputDigest, estimatedInputTokens, contextWindow, replNamespace?: ReplNamespaceStatus, retryOfCallId? }` | Attributes the exact initial or provider-overflow model attempt to its retained provider candidate. Attempts pin the exact cold or warm named REPL namespace status included in provider input. Only typed provider-confirmed overflow may create a later attempt over a strictly smaller candidate and an explicit transcript-segment reset. |
...
`ReplNamespaceStatus` is `{ protocol: "agencity.repl-namespace.v1", state: "cold", epochId: null, epochName: null }` or `{ protocol: "agencity.repl-namespace.v1", state: "warm", epochId: UUID, epochName: adjective-noun-sixHex }`. The attempt pin and an epoch-change failure describe process-local status; they are not heap snapshots or recovery data. `CellCommitted`, ordinary `CellFailed`, and `CellAbandoned` retain outcomes, not bindings. Event replay, branching, synchronization, gate evaluation, and export never reconstruct the namespace.
...
A new non-legacy `MailboxMessageSent(mode: "queue")` has one public queued-run identity derived as a versioned deterministic function of its immutable mailbox message ID. This is an API derivation, not a new event field or mutable table column. Exact sender/branch intent-key reuse therefore returns the same message and run IDs. Before `AgentRunRequested` exists, result lookup reports queued with zero steps; delivery failure reports failed without claiming that a run was admitted. Once the derived run is retained, lookup delegates to its canonical `AgentRun*` history. `steer` and retained legacy `followUp` rows expose no independent run identity.
...
A cell's external effects have their own lifecycle and can outlive an abandoned cell. Do not infer that abandonment rolled them back. Recovery never synthesizes cell-failure links: `CellAbandoned` remains uncertain even when nearby effect outcomes exist.
...
| `GovernedRefinementProposed` | Immutable profile or harness proposal, proposer/origin, expected target, replacement, reason, predicted effect, evidence, optional post-activation objective evaluation intent, optional revised-proposal link, and stable fingerprint. Automatic-refiner admission requires the evaluation; direct owner or agent proposals may omit it. |
