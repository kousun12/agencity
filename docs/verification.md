...
These tests cover UTF-8, item, byte, route, activity, queue, replay, and backward conversation-page bounds; chronological user/assistant and model-action projection; breadth-first family discovery and placeholders; exact-route mailbox aggregation; status derivation; stale-generation rejection; committed-event deduplication; provisional-progress cleanup; inert checked-in assets; default Route details Conversation selection; CLI option and port admission; managed-service availability and capability classification; the narrow read-method allowlist; snapshot validation; fragment/session authentication; exact Host, fetch metadata, Origin, no-CORS, no-store, and CSP behavior; browser attachment release; shared process-wide selection; service replacement; live child discovery; source-checkout ephemeral/explicit port behavior; signal shutdown; no fresh-workspace state creation; and linked asset resolution.
...
The first command installs the pinned Playwright Chromium prerequisite once. The journey covers token exchange and fragment removal, refresh, root selection, family rendering, the default Route details Conversation section with user and assistant messages, live child/message updates, lazy detail, managed-service replacement, and final-browser disconnect. It is deliberately not part of `bun run verify`. If Chromium is unavailable, the script fails with installation guidance. If the command was not run, report the browser journey as unverified rather than passed.

On August 21, 2026, the Conversation revision passed 52 deterministic observer tests with 0 failures and 0 skips. Source-checkout and isolated linked-asset cases passed. The opt-in Playwright Chromium journey passed 1 test with 0 failures and 0 skips through the actual foreground CLI, browser, and server against a protocol-compatible managed fixture.

Repository-wide `bun run verify` passed typecheck and architecture, 1,310 deterministic core tests with 3 gated or platform skips, and 13 end-to-end tests with no skips. The acceptance stage reported 23 passes, 1 real-provider skip, and 4 pre-existing `explicit-ai-agents.test.ts` fixture/service-connectivity failures. No Observe test failed. `git diff --check` passed. An earlier independent review of the first Observe version found no blocking code defects and independently passed typecheck, architecture, and 45 focused observer tests.
...
crosses scored tasks. This prompt revision has model-free coverage; recorded
paid RuneBench results predate it.
