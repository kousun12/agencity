...
This acceptance suite is intentionally non-interactive. The `test:core` groups separately cover deterministic full-screen renderer frames, input, and resize behavior in unit tests, plus a linked-executable pseudo-terminal task, commit, and detach path in `bun run test:e2e`.
