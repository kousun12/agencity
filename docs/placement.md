...
### Console REPL placement

Each exact-session-and-branch REPL namespace lives only inside its console worker and remains available while that worker survives. It is not part of `AgentStorage`, `agencity-relational-rpc-v1`, or any placement capability.

Local and remote relational placements use the same process-local namespace semantics. No placement stores, restores, synchronizes, exports, or transfers bindings or heap identity. Worker loss starts a fresh namespace; state and artifacts remain the supported recovery persistence.
