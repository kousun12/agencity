...
| Agent-profile inspection | Supported | The supervisor, HTTP protocol, and `AgentClient` expose bounded active/history reads. Exact prompt text requires an explicit full-detail request. The TUI has no profile inspector or editing controls. |
| Automated profile refinement governance | Unavailable | No public profile proposal, sealed reviewer, activation, terminal-delivery, or rollback path is implemented. Existing memory/prompt-note/skill/spec refinement retains its current governance behavior. |
| Installed profile-governance journey | Unavailable | Linked-product acceptance does not yet demonstrate profile proposal, independent approval/rejection, later-invocation activation, rollback, detach, or recovery. Runtime/profile tests are not a substitute for that product journey. |
