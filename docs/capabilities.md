...
| Capability | Status | Current behavior |
|---|---|---|
| OpenAI | Conditional | Built-in Vercel AI SDK OpenAI Responses API transport with provider storage disabled; requires a stored/programmatic key or `OPENAI_API_KEY` and a canonical `openai/...` model ID. |
