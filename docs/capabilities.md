...
| Provider/model onboarding | Supported | Interactive missing configuration uses keyboard typeahead for provider display names/IDs and fuzzy model display-name/canonical-ID search, with Up/Down, Enter, Backspace, Escape, bounded visible rows, and explicit exact manual canonical rows. Direct OpenAI and Anthropic enforce matching creator namespaces; Gateway accepts any valid creator. Display names remain presentation while canonical IDs are durable. Hidden credentials may remain after later picker cancellation; cancellation writes no model preference or root. |
