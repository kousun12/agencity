...
| Durable raw AI generation | Supported | `ai.generateText` and `ai.generateObject` perform exactly one provider generation over explicit prompt/messages and explicit bounded context. They retain exact dispatch, schema, provider input, usage, budget, cancellation, timeout, and recovery provenance without creating agent-family records. Results remain inline; oversized output fails. |
