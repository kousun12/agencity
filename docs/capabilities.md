...
| Full-screen terminal UI | Supported | The OpenTUI client consumes the public snapshot/event protocol and renders committed Markdown plus syntax-aware fenced code and retained TypeScript cells. It interleaves each compact run status after its initiating user task, provides expandable bounded cell outcomes, a fixed prompted composer, width-prioritized status/action hints, an idle full-width conversation, contextual wide/narrow inspectors, explicit height modes, responsive direct-child browsing, exact parent/child branch navigation, detach/catch-up, and a plain non-TTY transcript fallback. Unsupported fenced languages use plain-code rendering. Family opening is observational and disabled during historical inspection. |
