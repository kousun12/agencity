...
`ManagedServiceConfiguration.consoleRssRecycleThresholdBytes` is a positive
integer that defaults to 512 MiB. It also participates in the discovery
configuration hash and is exposed by the product CLI as
...
Structured requests use response-aware `agencity.model-dispatch.v2`, immutable `agencity.provider-input.v2`, and `agencity.model-effect-output.v2`. `buildProviderInputCandidate` produces the exact provider-neutral text, assistant tool-call, and tool-result messages; fixed formal tool declarations and order; policy; token-relevant options; cache contract; and dispatch/endpoint/capacity provenance used by both `estimateProviderInputCandidate` and execution. Recovery reconstructs and digest-checks that exact candidate. Unknown capacity remains explicit and imposes a 512 KiB complete-candidate ceiling with a 384 KiB compaction target. A custom provider's complete structured output is credential-checked across all fields and fails closed before return or persistence.

Autonomous runs build an initial attributable transcript segment. Every continued request in that segment preserves the prior candidate's message list as an exact prefix and appends the native assistant tool call or bounded rejection, the corresponding durable tool result or rejection observation, any changed durable-state delta, and one next-action message. Context compaction starts a new attributable segment and cache boundary; later requests resume append-only growth from that reset.

Direct OpenAI uses the Responses API with `store: false` and no `previous_response_id`. A v2 autonomous candidate carries the session/branch-stable deterministic prompt-cache key, `openai-explicit` mode, a 30-minute TTL, and explicit breakpoints on supported input-text boundaries. The fixed required tool schema and order are sent with parallel tool calls disabled. In a very long segment, the provider currently considers up to its 50 most recent breakpoints for cache reads; prior breakpoints are read-only and only the latest four may write on one request. Cache read/write token counts are retained in model usage diagnostics when reported, but budget debit remains `inputTokens + outputTokens`.
...
Snapshots and operational tables are projections. `agent_profile_versions`, `workspace_agent_profiles`, and migration-019 outbox origins rebuild from schema-version-6 events; recursive-handle and context projections rebuild their retained profile, effective-prompt, and provider-input provenance. Historical rebuild is deterministic and never re-executes effects.
