...
Profile, memory, prompt-note, skill, and subagent-specification revisions use one ordinary governance path. An authorized self, direct creation-family parent, workspace owner, or local automatic refiner records an immutable proposal. Deterministic validation checks scope, authority, bounds, secrets, compatibility, evidence, rendering, and expected-version state before one separate sealed reviewer runs. The reviewer uses the proposing route's current model and receives frozen product-constitution, review-policy, target, evidence, relationship, runtime-boundary, and dispatch inputs. Workspace-charter and user-constraint components are not configurable and are pinned as `null`; ambient repository guidance is not silently promoted to reviewer authority. A caller cannot select the reviewer, edit the decision contract, approve content, or widen runtime authority.
...
Reviewer approval establishes consistency with the frozen policy, not empirical improvement. Candidate/control allocation and outcome evaluation remain attributable advanced mechanisms but are not mandatory authority for the ordinary automatic application path.
