...
## Agent relationships, raw generation, and projections

Root agents and delegated subagents use the same `SessionCreated` event and `AgentState`. Child creation stores `parentSessionId`, `parentBranchId`, `rootSessionId`, `depth`, and `taskId`; storage validates those fields against the parent and the task row in the same append transaction. The public composition root exposes `Supervisor.agents`, `.documents`, `.ai`, `.goals`, `.heartbeats`, and `.schedules`. Their returned handles contain only JSON domain values and are returned only after the creating event batch commits.
...
Public agent invocation pins either normal text output or one restricted declared-object schema before the child model runs. `sdk.agents.run` and `runMany` await the full child lifecycle and return the retained typed result; `spawn` and `spawnMany` always start detached runnable work and return durable handles. Result lookup, terminal notices, task projection, and the calling cell resolve the same `AgentRunResultCommitted` reference. A worker or client disconnect therefore loses only handle delivery or an in-memory wait, not task identity. Awaited admission reserves branch-aware console capacity before creating the child, while detached work can queue. Output schemas constrain data shape but cannot select sealed response contracts, provider tools, dispatch, capabilities, permissions, or runtime authority.

Raw AI generation is separate from agent delegation. `AiGenerationService` admits exactly one text or declared-object provider request, freezes only explicit prompt/messages and bounded context references, reserves caller-narrowing budget, and records exact dispatch, provider input, schema, result, usage, and effect provenance. It creates no child task, session, profile, mailbox, or family record. `ai.generateText` and `ai.generateObject` are available directly and under `sdk.ai`; the former model-facing `rlm` surface is absent. Hard inline result limits fail closed without artifact substitution. Stable idempotency keys recover the same generation, pending cancellation suppresses outbox execution durably, active cancellation follows the authoritative effect outcome, and interrupted non-idempotent execution becomes `unknown` with conservative reservation debit. Raw and autonomous model effects share the configurable `providerConcurrency` limiter. The retained recursive-model service remains internal for historical records and sealed workflows.
...
`AgentProfileVersionCreated` and `AgentProfileActivated` provide session-wide revision. Migration 017 adds sealed-governance and restoration projections. Profile control events use the session's initial branch instead of a separate workspace stream address. Storage enforces that address and compares `expectedActiveProfileVersionId` against the mutable `workspace_agent_profiles` projection in the append transaction. `agent_profile_versions`, `workspace_agent_profiles`, `governed_refinement_proposals`, and `refinement_restorations` rebuild from canonical events in global cursor order. Runtime profile lookup reads the complete session history, so activation governs later invocations on every branch while historical branch history remains unchanged.
...
Profile, memory, prompt-note, skill, and subagent-specification revisions use one ordinary governance path. An authorized self, direct creation-family parent, workspace owner, or local automatic refiner records an immutable proposal. Deterministic validation checks scope, authority, bounds, exact registered credential values, compatibility, evidence, rendering, and expected-version state before one separate sealed reviewer runs. The reviewer uses the proposing route's current model and receives frozen product-constitution, review-policy, target, evidence, relationship, runtime-boundary, and dispatch inputs. Version-3 frozen input gives every proposal deterministic redacted evidence excerpts under one 32 KiB aggregate budget and retains canonical/redacted payload and excerpt digests, exact byte counts, truncation, and redaction provenance. Repository instruction content and exact supervisor-registered credential values are removed before excerpting. Workspace-charter and user-constraint components are not configurable and are pinned as `null`; ambient repository guidance is not silently promoted to reviewer authority. A caller cannot select the reviewer, edit the decision contract, approve content, or widen runtime authority.

An approval is revalidated against current state. Profiles and non-skill harness targets then apply atomically with their immutable version and terminal decision. Storage independently checks that a later profile version, activation, reviewed proposal, decision, expected prior version, replacement content, and application event agree exactly; initial admission and exact rollback use separate atomic patterns. Skills enter an idempotent staged path: candidate creation is retained, compile and declared runtime tests execute through the outbox, and only a passing retained report permits activation. Rejection, malformed output, failure, unknown outcome, or compare-and-swap conflict activates nothing. The sealed reviewer has frozen token, cost, turn, and wall-time limits and cannot inherit unbounded parent defaults. Waiting callers receive the terminal record directly; detached and automatic proposals receive one idempotent route notice. Recovery scans oldest-first in pages of 200 until all nonterminal work and terminal undelivered notices have been considered. A rejected proposal is immutable, and bounded reproposal creates a new identity in the same original principal/origin/target chain. Rollback creates and activates a new immutable version containing the exact earlier approved content; agent evidence must be visible on the caller branch lineage, while workspace-owner evidence may come from the same workspace. Changed content is a new proposal.

Reviewer approval establishes consistency with the frozen policy, not empirical improvement. Automatic trajectory proposals retain their required objective evaluation in proposal identity, reviewer input, inspection, and application history. This is post-activation evaluation intent, not an activation gate. Direct proposals may omit it. Candidate/control allocation and outcome evaluation remain attributable advanced mechanisms but are not mandatory authority for the ordinary automatic application path; generated-skill compile and declared runtime tests remain the only additional target-specific activation check.
...
These declarations have no execute callback, provider-hosted execution, or provider-managed loop. Agencity accepts exactly one completed permitted call per step and may carry that call plus its durable result into the next append-only provider request. Supplemental text is bounded diagnostic evidence only; it is never parsed for JSON or TypeScript. Zero, multiple, malformed, truncated, oversized, unknown, refused, or incomplete calls execute nothing and become typed contract violations.
...
Run, step, context, call, effect, action, and cell IDs are stable across recovery. `AgentRunStepStarted.observationEventIds` is the complete exact-once canonical delivery ledger. The provider-facing derivation gives a terminal cell ownership of successful cell effects, adds a compact effect manifest, and omits duplicate successful effect payloads; failure, cancellation, and uncertainty stay actionable. This projection is limited to 56 KiB per item and 64 KiB per dependent step without rewriting the ledger. A pending unclaimed model effect drains once; a retained succeeded outcome finalizes without a second provider call; a lost started non-idempotent effect becomes unknown. Started cells are abandoned rather than replayed. Budget admission uses the existing `>=` limits, and a generated finish may be accepted at the exact turn boundary while a new effectful cell is not.

The canonical writer accepts event schema 6, reducer 22, `agencity.model-dispatch.v2`, `agencity.provider-input.v2`, and `agencity.model-effect-output.v2`. This is an explicit pre-release reset boundary: workspaces containing schema versions 1, 2, 3, 4, or 5 reject with reset guidance before migration, decoding, projection, synchronization, or recovery. The provider-input candidate retains exact provider-neutral text, assistant tool-call, and tool-result messages; fixed formal tool declarations and order; request and cache policy/options; dispatch/endpoint/capacity provenance; digest; and byte count. The effect output retains one full accepted formal input. Completion and action events carry result digests, input digests, provider call identity, model-call references, invocation prompt provenance, and optional provider cache read/write usage diagnostics; rejected raw arguments are not retained.
...
| range read | yes | yes | Zero-based half-open ranges are limited to 64 KiB. Both adapters verify the complete object before slicing; neither implements an optimized partial-object integrity scheme. |
...
Artifact registration is canonical; bytes are outside the database. Local streaming producers use owner-only staging directories. Startup removes stale staging owned by dead processes. Shell spill registers its CAS object and referencing effect outcome atomically; oversized cell-result registration is atomic with the cell terminal batch. A crash or failed append can leave an unreachable CAS object, which is not evidence of success. General artifact garbage collection remains unavailable.
...
|---|---|---|
| `FileExecutor` | bounded one-based `read`, atomic `write`, exact-one `replace`, file `delete` | Read pages allow at most 2,000 lines, 2 KiB per line, and 48 KiB total; digest-pinned continuation detects mutation. Root/symlink checks and directory-delete refusal confine the typed adapter, not arbitrary process code. |
| `ShellExecutor` | `run` with cwd and timeout | Streams scrubbed output into 12 KiB head/12 KiB tail previews per stream. Complete output up to 32 MiB spills to local CAS; crossing the limit or losing staging returns explicit truncation without a false artifact. It uses `/bin/sh -c`, retains ambient authority, and is normally non-idempotent. |
| `ModelExecutor` | `complete` through registered provider; optional provider streaming | Gateway, direct OpenAI, and direct Anthropic use one Vercel AI SDK adapter core with transport-specific factories. Direct OpenAI uses the Responses API with `store: false`, no `previous_response_id`, and reasoning summaries disabled; reasoning effort and required function tools remain explicit. Autonomous requests use a deterministic session/branch cache key, explicit cache mode, a 30-minute TTL, explicit supported text-boundary breakpoints, fixed tool schema/order, and no parallel calls. Its endpoint identity includes `/v1/responses`, so recovery rejects retained Chat Completions dispatches instead of executing them under changed semantics. Every request retains an immutable `ModelDispatch` containing complete configuration, reasoning capability decision, catalog digest, and execution endpoint identity; recovery rejects endpoint drift instead of reinterpreting it. Live deltas are ephemeral and only the full returned response enters the durable success batch. Calls are non-idempotent, so lost outcomes become unknown rather than a blind second generation. Echo remains an internal non-streaming test fixture. |
...
`RemoteSandboxExecutor` and its HTTP server adapter provide capability negotiation, typed outcomes, and transport-level cancellation for a server-owned executor. The version-1 RPC has no canonical artifact-transfer capability, so a remote result containing a spilled artifact reference is rejected rather than presenting an unreachable recovery path. The server operator asserts the advertised filesystem, network, and host-isolation policy; the client does not attest or independently verify that isolation. This repository supplies the adapter and conformance coverage, not hosted sandbox infrastructure. Per-provider distributed concurrency, resource quota enforcement, and browser execution remain unavailable. Every executor placement must preserve IDs, request-before-effect ordering, four-way outcomes, cancellation disclosure, and idempotency semantics.
...
An autonomous run starts one attributable provider transcript segment containing
the effective system prompt, durable conversation inputs, and a stable durable
context baseline. Every continued request preserves the prior message list as
an exact prefix, then appends the native provider-neutral assistant tool call or
bounded rejection, its durable tool result or rejection observation, a
top-level durable-state delta when facts changed, and one next-action message.
This replaces the rebuilt `AGENCITY DURABLE RUN STEP` sliding trajectory as the
ordinary continuation contract. Active working values remain in the durable
baseline and later deltas, so a small replaceable checkpoint preserves progress
without rebuilding earlier provider-visible messages.

Compaction is a segment and cache reset. It creates attributable derived context,
starts a new stable transcript baseline with an explicit reset reason, and then
resumes append-only growth. Direct OpenAI marks each supported next-action text
block as an explicit breakpoint; function-call outputs are retained as native
tool results but are not relied upon as cache-write boundaries. In very long
segments the provider currently considers up to its 50 most recent breakpoints
for reads, while prior breakpoints are read-only and only the latest four may
write on a request.
...
`entryId` names a durable memory/prompt-note/skill/spec; `versionId` names immutable content. An entry keeps latest and active pointers independently. The ordinary path is immutable proposal, deterministic validation, sealed review, application-time validation, and automatic application. Exact rollback appends a restoration version rather than editing content.
...
The retained ADR-0002 candidate allocation, exposure, observation, approval, and promotion APIs remain available for advanced and legacy-compatible evaluation. They bind objective observations to exact session, branch, task, version, and predeclared metric/test evidence, but they are not the ordinary activation path under ADR 0012. New integrations should use governed proposals, terminal records, and exact-content rollback.
...
Generated skills are the additional hard gate: reviewer approval stages a candidate version, then Bun compilation and every declared runtime case execute in a separate process through the ordinary durable outbox. Only a passing report permits activation. The configured permission-name allowlist is checked at validation, activation, testing, and invocation. Invocation repeats compilation for the exact pinned immutable source and records both effect and skill-version linkage. This is trusted-local process isolation, not a new sandbox.
...
Legacy candidate promotion remains scope sensitive for callers that deliberately use that advanced API. Ordinary governed refinement does not require candidate promotion or per-proposal human approval. Its authority comes from scoped proposal admission, deterministic checks, sealed review, application-time revalidation, exact terminal delivery, attributable later outcomes, and rollback.
...
`ContextWindowController` is a pure admission policy shared by the diagnostic `ModelLoop` and `AgentRunService`. Estimation, execution, and recovery consume the same immutable `agencity.provider-input.v2` candidate. The deterministic `provider-input-utf8-bytes-per-4-tokens-v1` estimator covers messages, tool schemas, cache and selection policy, and token-relevant options; it is not provider-reported usage. Capacity provenance remains exact. When capacity is unknown, a 512 KiB complete-candidate hard ceiling and 384 KiB compaction target prevent unbounded admission without pretending to know provider capacity. The controller first asks `CompactionService` for eligible narrative material. A completed canonical compaction starts a new provider-transcript segment from its derived summary. If no canonical narrative is eligible but a prior autonomous transcript exists, the controller instead seals an attributable segment boundary whose materialized context references the exact prior model request, action, and observations; it then rebuilds from bounded recent trajectory and current durable state. Both paths accept one cache reset and resume append-only growth. A provider overflow retry requires an adapter-supplied typed classification and a strictly smaller estimate; `AgentRunModelAttemptStarted` gives every retry a new context/call/effect identity and pins the candidate version/digest. Provider-reported cache read/write tokens remain diagnostics and do not change budget debit, which continues to use input plus output tokens.
