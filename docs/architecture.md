...
`HeartbeatService` and `ScheduleService` only define/tick recurring work and append the durable wake queue. `ScheduleService` owns a replaceable `WakeCoordinator` seam that currently targets the embedded `AgentRunService`; execution-owner checks remain in run admission. This does not imply an always-on detached service.
