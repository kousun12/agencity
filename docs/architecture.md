...
An approval is revalidated against current state. Profiles and non-skill harness targets then apply atomically with their immutable version and terminal decision. Skills enter an idempotent staged path: candidate creation is retained, compile and declared runtime tests execute through the outbox, and only a passing retained report permits activation. Rejection, malformed output, failure, unknown outcome, or compare-and-swap conflict activates nothing. Waiting callers receive the terminal record directly; detached and automatic proposals receive one idempotent route notice. A rejected proposal is immutable, and bounded reproposal creates a new identity linked to the rejection. Rollback creates and activates a new immutable version containing the exact earlier approved content; changed content is a new proposal.
