...
The persistent console REPL namespace is process memory only. It is never written to the workspace database, synchronized, exported, or backed up. State and artifacts are the supported persistence for values required after worker loss.
...
A backup never captures a live REPL namespace. Plan recovery around canonical state and artifacts.
...
Raw provider key values are not exported from `auth.json`. Workspace source files, external service state, and live REPL namespaces are also not included.

For schema-version-6 workspaces, `events.jsonl` carries initial and historical agent profiles, invocation profile pins, context/model-call effective-prompt and provider-input-v2 provenance, append-only provider-transcript segments and resets, typed effect origins, complete observation ledgers, bounded-output artifact references, governed proposals, frozen reviewer inputs and dispatch, reviewer-child links, decisions, terminal notices, and restoration provenance because those values are canonical event payloads. The rebuildable profile, outbox-origin, and governance projection tables are not separate authority that must be exported beside their source events.
...
- The current workspace format accepts event schema version 6, reducer version 22, provider input version 2, model dispatch version 2, bounded output version 1, and model effect output version 2.
- This is an explicit pre-release reset boundary. Workspaces containing event schema version 1, 2, 3, 4, or 5 are rejected with reset guidance and are not decoded, upcast, synchronized, projected, or recovered. Profile model-catalog caches may be discarded and rebuilt.
...
- Migration 019 adds `outbox.origin_json`. The canonical schema-6 `EffectRequested.origin` remains authoritative; rebuild and recovery copy and validate the exact closed origin rather than infer it from event order.
