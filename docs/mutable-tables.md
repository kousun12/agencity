...
| `console_scratch_cache` | `operational-projection` | `mutable` | Replaceable, same-device, exact-session-branch JSON checkpoints for the console's best-effort scratch object. Fenced local operations validate the latest direct-branch `CellCommitted` source, integrity, expiry, and workspace quotas transactionally. Rows are safe to delete and are excluded from generated SQL, canonical rebuild, synchronization, export, and remote placement. |
...
| `mailbox_messages` | `rebuildable-projection` | `mutable` | Shared sender/recipient query projection of canonical send, mailbox delivery, context delivery/failure, and acknowledgement events. It retains intent/reply/task/artifact/relationship/message-mode/context-run fields plus queued/context-delivered/acknowledged/failed receipt state. `message_mode` retains new `steer`/`queue` intent; nullable legacy `follow_up` and `follow_up_run_id` columns preserve earlier schema-5 history and consuming-run linkage. Public projections expose `mode`, `legacyFollowUp`, and `contextRunId`. Paired retained events without explicit context-receipt fields rebuild as context-delivered for compatibility; deletion/replay restores the exact row without delivering context or scheduling work. |
...
| `recursive_model_handles` | `rebuildable-projection` | `mutable` | Current recursive-call lookup/status projection, including retained response admission, migration-016 `profile_pin_json`, and structured-result recovery fields; task, child session, model, response contract, profile pin, and terminal transitions remain canonical events. |
...
### `console_scratch_cache`

This local cache stores only independently serializable scratch properties after a successful cell commit. A row is keyed by the exact session and branch, bound to the originating device and latest direct-branch `CellCommitted` event, and protected by content and full-row integrity digests. Runtime load, write, clear, expiry pruning, and least-recently-used eviction require the current workspace and root execution fence in the same transaction.

The cache is limited to seven days, 64 branch rows, and 16 MiB of encoded rows per workspace. Corrupt, expired, device-mismatched, and least-recently-used rows may be discarded. Projection rebuild leaves it untouched; independent-session erasure removes matching rows; whole-workspace deletion removes it with the workspace database. It is private to generated analytical SQL and absent from synchronization, product export, and remote relational contracts.

### Recursive session projections

`tasks`, mailbox/terminal delivery rows, document/chunk/input-set rows, goals/gates with their completion workspace pins, heartbeats, and recursive model handles are updated in the same transaction as their canonical event. `response_admission_json` and migration-016 `profile_pin_json` in `recursive_model_handles` are mutable projection data reconstructed from `RecursiveModelStarted`, not new sources of truth. Recursive-handle input/provenance/hash, response admission, profile pin, and outcome/structured-result/artifact columns are rebuildable query state; the corresponding `RecursiveModel*` events remain authoritative. Recovery before the first model request reads the exact retained response contract, capability seed, and profile pin instead of resolving current values again. A sealed refinement result is validated against that admission and the exact child completion; public recursive results remain text. `rebuildOperationalProjections()` deletes these mutable rows and replays only their source events; it never re-executes a model, gate, tool, heartbeat callback, or subagent. Document content is duplicated in a query-friendly table, but the `DocumentChunkAdded` event remains authoritative.
