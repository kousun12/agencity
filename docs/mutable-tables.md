...
| `recursive_model_handles` | `rebuildable-projection` | `mutable` | Current recursive-call lookup/status projection, including migration-015 `response_admission_json` and structured-result recovery fields; task, child session, model, response contract, and terminal transitions remain canonical events. |
...
`tasks`, mailbox/terminal delivery rows, document/chunk/input-set rows, goals/gates with their completion workspace pins, heartbeats, and recursive model handles are updated in the same transaction as their canonical event. Migration 015 adds `response_admission_json` to `recursive_model_handles`; the column is mutable projection data reconstructed from `RecursiveModelStarted`, not a new source of truth. Recursive-handle input/provenance/hash, response admission, and outcome/structured-result/artifact columns are rebuildable query state; the corresponding `RecursiveModel*` events remain authoritative. Recovery before the first model request reads the exact retained response contract and capability seed instead of resolving current capability again. A sealed refinement result is validated against that admission and the exact child completion; public recursive results remain text. `rebuildOperationalProjections()` deletes these mutable rows and replays only their source events; it never re-executes a model, gate, tool, heartbeat callback, or subagent. Document content is duplicated in a query-friendly table, but the `DocumentChunkAdded` event remains authoritative.
