...
| `governed_refinement_proposals` | `rebuildable-projection` | `mutable` | Migration-017 query state for the sealed-governance lifecycle, frozen constitution/policy/target/evidence/current-model dispatch, exact reviewer child/decision, application outcome, and terminal-delivery pointer. Canonical governed-refinement events remain authority. |
