...
This local cache stores only independently serializable scratch properties after a successful cell commit. A row is keyed by the exact session and branch, bound to the originating device and latest persisted direct-branch `CellCommitted` event, and protected by content and full-row integrity digests. Runtime load, write, clear, expiry pruning, and least-recently-used eviction require the current workspace and root execution fence in the same transaction. A filtered, validated candidate whose schema version and serialized digest match the current valid row returns `unchanged` without modifying any column or running quota eviction, so the prior payload, source provenance, timestamps, TTL, access and integrity metadata, and quota position remain exact.
