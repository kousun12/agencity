...
Only a committed terminal event and its committed exports become required later state. While the exact-session-and-branch REPL worker lives, top-level variables, functions, classes, imports, module instances, and object identity remain available across cells. That namespace is noncanonical and may disappear on cancellation, RSS recycling, worker/service/process loss, or branch change. Each worker generation has an authoritative random epoch ID and a readable name. Autonomous model input and its attempt event retain whether the branch was cold or the exact warm epoch the model saw.
...
Before an accepted autonomous cell runs, Agencity compares the attempt's pinned status with the worker-pool status observed at admission. A mismatch records `CellFailed.failure.code = REPL_EPOCH_CHANGED` with expected/current status and reconstruction guidance. A retained action from before epoch pinning fails the same way with `expected: null`. The submitted source does not run, so the next model step can rebuild only the required bindings from state, artifacts, or current inputs. Recovery never replays retained cell source automatically because arbitrary replay could repeat a file edit, command, network call, or model generation.

## Startup sequence

Unless `recover: false`, `Supervisor.open` performs:

Before this sequence, storage admission verifies that every retained event uses schema version 6 and reducer version 25. This is an explicit pre-release reset boundary: a workspace containing schema version 1, 2, 3, 4, or 5 fails closed with reset guidance before product migration, row decoding, projection, synchronization ingestion, or recovery. The runtime does not upcast, rewrite, or delete that history. Back up or move aside the incompatible workspace state before creating a fresh schema-version-6 workspace.
...
force signal to an authenticated surviving executing group. A group containing
only Linux `Z` or `X` process states is terminal even though signal-zero still
finds its process-group ID. Failed TERM/KILL cleanup appends
`ManagedProcessStopFailed` with the reason, error, attempted groups, time, and
surviving executing-or-unconfirmed groups plus a monotonic attempt number, so
inspection does not collapse the failure to a generic running state. Every
known group receives a cleanup attempt even when an earlier group fails.
Recovery retains any scrubbed log prefix available from owner-only staging and
records `unknown`; it never restarts the command. A worker recycle does not end
a healthy managed process; the workspace service remains its owner and a
serialized JSON handle can inspect or stop it from a later worker.
...
Structured model requests commit `agencity.provider-input.v2` in `ModelCallRequested` and its compact admission identity with the exact context before provider execution. It contains provider-neutral text, assistant tool-call, and tool-result messages; sealed tool declarations and fixed order; selection/parallel and cache policy; token-relevant options; dispatch/endpoint/capacity provenance; digest; and exact byte count. Estimation, execution, and recovery all use that exact candidate.

For an autonomous continuation, recovery loads the prior successful candidate, authoritative `agencity.model-effect-output.v2`, canonical action, exact-once observations, and retained transcript-state baseline. It preserves the prior message list as an exact prefix and appends the native tool call/result or bounded rejection/observation, changed durable-state delta, and next-action message. It does not require a live REPL worker, provider response object, mutable catalog, provider storage, or `previous_response_id`. A missing or mismatched prior candidate, segment baseline, dispatch, version, or digest fails closed instead of rebuilding earlier messages from current projections.

Compaction creates an attributable transcript-segment and cache reset. Recovery rebuilds from the compacted context and explicit reset reason, then resumes append-only growth in the new segment. Direct OpenAI request recovery preserves the session/branch-stable deterministic key, explicit mode, 30-minute TTL, supported text-boundary breakpoints, fixed required tools/order, and parallel-call suppression. Provider-reported cache read/write token counts remain diagnostics; budget recovery continues to debit input plus output tokens. Provisional formal arguments never execute from stream callbacks. Contract-violation output retains bounded evidence but not rejected argument bodies. Guard-aborted responses debit a conservative token estimate with zero provider cost, so invalid output does not evade budget accounting.
...
| REPL status changes after a model call but before its cell executes | `CellFailed` with typed `REPL_EPOCH_CHANGED`, expected/current cold-or-warm epoch status, and no evaluated source. | Deliver the failure once to the next step and rebuild required bindings without replaying prior effectful cells. |
...
| Agent invocation admitted, parent worker or client disappears before handle/result delivery | Parent task, child session/profile, run ID, contract, and idempotency key are canonical. | Retry transient same-process run advancement with the same ID; otherwise recover the same handle/result at startup. Do not admit a second child. |
...
- profile versions, active pointers, invocation pins, effect origins, provider-input admission, provider-transcript segments, and effective-system-prompt provenance rebuild from canonical schema-version-6 events;
