...
The leading ID is shorthand for `--env.taskset.id`. A harness belongs to an agent — `--env.agent.harness.*` on the single-agent env, `--env..harness.*` on a multi-agent one (there is no run-level `--harness.*`):
