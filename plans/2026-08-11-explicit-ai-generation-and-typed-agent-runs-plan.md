...
### 2026-08-11 — Branch-aware console execution pool
- Completed: Replaced shared console-process serialization with a bounded exact-branch worker pool, separate resident and active-execution permits, atomic scope-aware capacity reservations for awaited children, queued detached work, per-branch scratch/stdout/process isolation, worker-scoped cancellation and recycling, bounded LRU retirement, pool-aware service status and quiescence, and shutdown that drains admitted RPC work before storage closes. Independent review hardened running-child retries, concurrent RPC permit reacquisition, reservation rollback, late release and close races, stopped-worker pruning, and worker-metadata bounds.
- Validation: Focused console-pool, console, managed-service, agent-run, family-agent, scratch, and recovery tests passed (150 tests, 0 failed); `bun run typecheck`; `bun run check:architecture`; `git diff --check`.
- Plan notes: The default pool retains at most 17 exact-branch Bun processes—one suspended caller plus the maximum 16-member awaited batch—and runs at most four generated cells actively. Awaited depth defaults to eight and cannot exceed durable session depth. Capacity failures use `CONSOLE_CAPACITY_EXCEEDED` before child admission; existing resident child scopes satisfy retries without consuming duplicate capacity.
- Remaining: Broad prompt guidance, public API cutover documentation, installed-product acceptance coverage, aggregate verification, and external checks remain.

### 2026-08-11 — Public cutover, guidance, documentation, and installed verification
- Completed: Removed the obsolete exported recursive-model protocol request, retained only supervisor-private recursive history/sealed operations, expanded version-8 generated guidance with deterministic/raw/awaited/detached decision rules and compiling examples, documented the `/ai/generations` and `/agent-invocations` lifecycles and security boundaries, reassessed tenacious goals, and added linked-product coverage for raw text, Zod object generation, typed text/object agents, detached `handle.result` and SDK result lookup, JSON-compatible durable handles, nested awaited execution, worker/service and client loss, and pre-admission capacity failure.
