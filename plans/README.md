...
- [Explicit AI generation and typed agent runs plan](./2026-08-11-explicit-ai-generation-and-typed-agent-runs-plan.md) — replace the misleading public `rlm` API with context-only `ai.generateText`/`ai.generateObject`, add awaited text or object-returning full agent runs, unify model selection, and keep cross-agent callable behavior on durable RPC.
- [First-run provider and model typeahead plan](./2026-08-10-first-run-provider-model-typeahead-plan.md) — completed shared fuzzy provider/model selection, explicit manual canonical IDs, credential-free configured-catalog discovery, pre-admission validation, and first-run `/model` convergence; deterministic aggregate and installed acceptance verification passed, while gated external integrations remain unverified.
