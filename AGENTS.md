...
- **Scratch** is an exact-session-and-branch, noncanonical console cache. Its warm object may hold ordinary runtime values while one worker survives; the managed file-local product may opportunistically restore independently serializable bounded JSON properties from a fenced operational cache. Clean scopes skip serialization and cache writes, while mutable reads remain conservatively dirty so nested changes cannot be missed. After supervisor-side sensitive filtering and candidate validation, a schema-and-digest match marks the warm scope clean without refreshing the valid cache row or its checkpoint provenance, timestamps, TTL, access metadata, integrity metadata, or quota position. Scratch does not transfer across parent, child, sibling, or forked work and is never task ownership, completion evidence, synchronization, export, automatic context, or a recovery requirement.
...
- disposable Bun TypeScript cells in a bounded exact-branch worker pool with separate resident-process and active-execution permits, deadlock-free capacity-reserved awaited children, queued detached children, isolated warm scratch/stdout/worker loss, dirty-tracked opportunistic fenced file-local JSON scratch checkpoints with no-write schema-and-digest deduplication in the managed product, final-expression or explicit-return observations, bounded safe inspection/logs, durable working values, retained cell history, read-only analytical SQL, content-addressed artifacts, 128 KiB cell-result IPC with streamed JSON artifact staging above that boundary, one-based bounded file pages, and exact bounded artifact byte ranges;
...
- durable root and child sessions, nuclear-family mailboxes with default queued sends and explicit non-waking steering, cancellation trees, recursive-model runtime handles, documents/input sets, goals, cached attributable gates, heartbeats, schedules, and wake queues;
