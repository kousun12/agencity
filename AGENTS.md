...
receiver split with a compact translated SDK quick start, batches the pinned API,
relevant learning, scored-skill wiki page, inventory, XP, and bounded live state
into one discovery cell, distinguishes upstream `rs-sdk` operational learnings
from the extracted skill/item/NPC/shop/quest wiki, and directs a proven non-zero
strategy toward deliberate bounded execution with compact durable observations.
A later
