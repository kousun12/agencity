...
- Managed service protocol revision 3 accompanies event schema 6. A revision-3 client may attach to a still-running revision-2 service so already-admitted work can finish; the TUI follows that service through cursor-pinned snapshot refresh instead of applying schema-5 events with the schema-6 reducer. Revision-2 clients cannot attach to revision-3 services, and other incompatible protocol ranges fail visibly.
