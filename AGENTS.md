...
- Generated execution guidance states the exact positional artifact API, immutable range bounds, and `agencity.bounded-output.v1` shell-result branches. It requires delegated work to be strictly narrower than the caller's assignment, requires every model-facing child admission to pass a concise stable human-readable name and preserve any exact declared identity, forbids recursive whole-task pass-through, and directs models to use the on-demand `sdk.agents.list()` nuclear-family projection before child admission when retained work may overlap. That call returns `{ items: FamilyAgentRecord[] }`, never a bare array. `runMany` and `spawnMany` accept and return arrays directly rather than `{ tasks }`, `{ results }`, or `{ items }` wrappers. Family status is not injected or polled on every model step.
...
- Healthy in-flight effects owned by an active run and ordinary active child tasks are not labeled as recovery items in the TUI footer. Background session-title uncertainty remains inspectable on its route but does not mark otherwise completed child work as family attention.
