...
without measured task progress, and provider-cache divergence at mutable budget
state. The current working tree removes the implicit production step ceiling,
...
deadline-aware completion guidance, and orders stable durable context before
mutable run state. These repairs have model-free coverage but no paid canary
verification.
