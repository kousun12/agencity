...
- Durable per-session agent profiles and invocation-level profile/effective-prompt pins are implemented runtime foundations under [ADR 0012](./docs/decisions/0012-durable-agent-profiles-automated-refinement-governance.md). Public profile proposal and revision commands, sealed governance review, automatic approved-version activation, durable proposal-result delivery, rollback, TUI profile controls, and the complete installed profile-governance journey are not implemented. Existing memory, prompt-note, skill, and subagent-specification refinement behavior remains the shipped governance path.
