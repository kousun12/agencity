...
- The ordinary task route drives the formal provider tool contract: every autonomous model call commits a required-tool-set dispatch, offers exactly `bun_console` and `finish`, and commits a digest-linked tool submission or typed contract violation before any action applies. No new action path asks for or parses raw action JSON from assistant text. Remaining formal-tool-contract-plan gaps: the retained clarification/permission action variants, pending-input events and route, and `waiting_for_user` state are unreachable through the formal contract but their code is deleted in Phase 5, together with pre-admission capability rejection and atomic blocked/failed `finish` final-message materialization; trajectory refinement still parses its review decision from assistant text until the Phase 6 structured migration; public docs and the superseding ADR remain Phase 8.
