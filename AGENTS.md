...
- Durable per-session agent profiles, invocation-level profile pins, sealed governance review, automatic approved-version activation, and durable proposal-result delivery are accepted architecture under [ADR 0012](./docs/decisions/0012-durable-agent-profiles-automated-refinement-governance.md) but are not implemented. Existing session admission, context composition, and refinement behavior remain the shipped behavior until the new runtime path and installed-product verification are complete.
