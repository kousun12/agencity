...
- Durable per-session agent profiles and invocation-level profile/effective-prompt pins are implemented under [ADR 0012](./docs/decisions/0012-durable-agent-profiles-automated-refinement-governance.md). Profile, memory, prompt-note, skill, and subagent-specification proposals use deterministic validation, one separate sealed reviewer selected from the origin route's current model, application-time revalidation, automatic approved-version application, exact terminal delivery, bounded reproposal, and exact rollback. Profile and non-skill application is atomic; approved skills activate only after durable compile and declared runtime tests pass. Self, direct-child, workspace-owner, and local automatic-refiner authority is enforced; siblings and unrelated agents cannot revise one another. The reviewer receives the frozen product constitution and policy, while unavailable workspace-charter and user-constraint configuration is retained as `null`; callers cannot select the reviewer or widen authority. Public protocol, `AgentClient`, Console SDK, route-relative `agencity profile`, and `/profile` controls expose inspection, proposal, wait/detach, history/diffs, reasons/guidance, notices, and rollback. Runtime, recovery, sync-divergence, export-audit, deletion-refusal, and focused lifecycle coverage have passed. Isolated linked-executable acceptance covers exact root and child profiles, old/new invocation pins, blocking approval, rejection, bounded reproposal, exact rollback, detached managed-service restart, deduplication, and no-ID inspection. Final post-hardening aggregate verification remains pending, so the complete plan is not yet claimed finished.
