...
- Console cells support REPL observation, bounded `inspect`, artifact spill, `state.list`, and retained `cells.list/get`. Top-level variables, functions, classes, imports, module instances, and object identity persist across cells while the exact-branch worker lives. Completed in-memory mutations remain after a runtime throw, but staged state and artifact writes do not commit. Parse/transpilation failure, cancellation, worker loss, or failure after execution but before canonical commit recycles the worker. Required recovery values come only from state or artifacts; fresh work may recompute from current external inputs, but retained source is never replayed automatically.
