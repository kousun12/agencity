...
- loopback HTTP/JSON and SSE surfaces with cursorless provider progress, snapshot-plus-cursor terminal waiting for protocol client agent and raw-generation convenience methods, a TypeScript API, a no-ID product CLI plus compatible diagnostic commands, and a protocol-backed terminal TUI;
