...
- loopback HTTP/JSON and SSE surfaces with cursorless provider progress, periodic branch-stream comment heartbeats, snapshot-plus-cursor terminal waiting for protocol client agent and raw-generation convenience methods, strictly read-only owner-validated service-manifest inspection and bounded exact-path passive discovery polling primitives, a TypeScript API, a no-ID product CLI plus compatible diagnostic commands, and a protocol-backed terminal TUI;
