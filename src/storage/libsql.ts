...
async appendEvents(rawEvents: readonly NewAgentEvent[], fence?: ProcessExecutionWriteFence): Promise<AgentEvent[]> {
  ...
  async #validateCanonicalAppend(tx: Transaction, event: AgentEvent): Promise<void> {
    ...
    if (event.type === "RefinementReviewRequested") {
      ...
      if (request.reviewId !== payload.reviewId || request.fingerprint !== payload.fingerprint || request.sessionId !== event.sessionId || request.branchId !== event.branchId || request.requestedScope !== payload.requestedScope || request.trigger.triggerId !== payload.triggerId || request.trigger.fingerprint !== payload.triggerFingerprint || !Bun.deepEquals(request.visibleSourceEventIds, payload.sourceEventIds) || !Bun.deepEquals(request.trigger.evidenceEventIds, payload.evidenceEventIds)) throw new ValidationError("Refinement review request event does not match its strict request envelope");
  ...
  async #applyOperationalRows(tx: Transaction, event: AgentEvent): Promise<void> {
    ...
    if (event.type === "RefinementReviewRequested") { const p = event.payload as EventPayloads["RefinementReviewRequested"]; await tx.execute({ sql: "INSERT INTO refinement_reviews(review_id,session_id,branch_id,fingerprint,mode,requested_scope,requested_scope_key,allowed_kinds_json,trigger_id,trigger_kind,trigger_fingerprint,trigger_key,nonterminal_key,evidence_event_ids_json,source_event_ids_json,source_snapshot_hash,source_through_cursor,instructions,request_json,status,created_event_id,last_event_id,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'requested',?,?,?,?)", args: [p.reviewId,event.sessionId,event.branchId,p.fingerprint,p.mode,p.requestedScope,p.requestedScopeKey,json(p.allowedKinds),p.triggerId,p.triggerKind,p.triggerFingerprint,p.triggerKey ?? null,p.nonterminalKey ?? null,json(p.evidenceEventIds),json(p.sourceEventIds),p.sourceSnapshotHash,p.sourceThroughCursor,p.instructions ?? null,json(p.request),event.id,event.id,event.committedAt,event.committedAt] }); }
  ...
  async loadSnapshot(sessionId:string,branchId:string):Promise<AgentState|null>{const r=await this.#execute({sql:"SELECT reducer_version,state_json FROM snapshots WHERE session_id=? AND branch_id=?",args:[sessionId,branchId]});if(!r.rows[0]||Number(r.rows[0].reducer_version)!==5)return null;return JSON.parse(String(r.rows[0].state_json)) as AgentState;}
