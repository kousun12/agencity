...
function rowToEvent(row: Row): AgentEvent {
  ...
  if (schemaVersion !== EVENT_SCHEMA_VERSION) {
    throw new ValidationError(
      `This workspace contains pre-cutover event schema version ${schemaVersion}. Reset local Agencity state before using schema version ${EVENT_SCHEMA_VERSION}; the retained row was not decoded or projected.`,
    );
  }
  return { cursor: cursorOf(localSequence), id: String(row.id), sessionId: String(row.session_id), branchId: String(row.branch_id),
    ...
    type: String(row.type) as EventType, schemaVersion, committedAt: String(row.committed_at), producer: String(row.producer),
...
function rowToRecursiveModel(row: Row): RecursiveModelRecord {
  const input = optionalJson(row, "input_json");
  ...
  async migrate(): Promise<void> {
    await this.#writes.run(async () => {
      ...
      const migrations = [
        ...
        { version: 21, name: "mailbox-message-modes", url: new URL("./migrations/021_mailbox_message_modes.sql", import.meta.url) },
        { version: 22, name: "ai-generations", url: new URL("./migrations/022_ai_generations.sql", import.meta.url) },
      ];
...
async appendEvents(rawEvents: readonly NewAgentEvent[], fence?: ProcessExecutionWriteFence): Promise<AgentEvent[]> {
  ...
  async #applyOperationalRows(tx: Transaction, event: AgentEvent): Promise<void> {
    ...
    if (event.type === "EffectRequested" && executionOwned) { const p = event.payload as EventPayloads["EffectRequested"]; await tx.execute({ sql: "INSERT INTO outbox(effect_id,session_id,branch_id,executor,operation,input_json,origin_json,idempotency_key,idempotent,status,requested_event_id,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)", args: [p.effectId,event.sessionId,event.branchId,p.executor,p.operation,json(p.input),json(p.origin),p.idempotencyKey,p.idempotent ? 1 : 0,"pending",event.id,event.committedAt,event.committedAt] }); }
...
export interface LibSqlScratchStoreOptions {
  readonly url: string;
  readonly deviceId: string;
...
interface ScratchCacheRowData {
  ...
  readonly savedNamesJson: string;
  readonly skippedJson: string;
...
interface EncodedScratchCacheRow extends ScratchCacheRowData {
  readonly encodedRowBytes: number;
  readonly rowIntegrityDigest: string;
}

/**
 * File-local implementation of the private ScratchStore contract. It opens a
 * separate connection so scratch remains outside AgentStorage composition,
 * while every runtime mutation validates canonical source and lease authority
 * in the same workspace-database transaction.
 */
export class LibSqlScratchStore implements ScratchStore {
  readonly #client: Client;
  readonly #writes = new LocalWriteQueue();
  readonly #deviceId: string;
  readonly #ttlMs: number;
  ...
  readonly #maxWorkspaceBytes: number;
  readonly #now: () => Date;
  readonly #latestSourceCursors = new Map<string, number>();
  #closed = false;

  constructor(options: LibSqlScratchStoreOptions) {
    assertExactFileUrl(options.url, "Scratch cache");
    if (!options.deviceId.trim()) throw new ValidationError("Scratch cache device ID is required");
    this.#deviceId = options.deviceId;
    this.#ttlMs = positiveScratchLimit(options.ttlMs, SCRATCH_STORE_LIMITS.ttlMs, "TTL");
    this.#maxBranches = positiveScratchLimit(
      options.maxBranches,
      SCRATCH_STORE_LIMITS.maxBranches,
      "branch quota",
    );
    this.#maxWorkspaceBytes = positiveScratchLimit(
      options.maxWorkspaceBytes,
      SCRATCH_STORE_LIMITS.maxWorkspaceBytes,
      "workspace byte quota",
    );
  ...
  async load(
    ...
    assertScratchScope(scope);
    return this.#writes.run(async () => {
      let restoredSourceCursor: number | null = null;
      const result = await this.#transaction<ScratchCheckpointLoadResult>(async (tx) => {
        const workspaceId = await this.#assertScopeFence(tx, scope, fence);
        const now = this.#timestamp();
        await this.#pruneExpired(tx, workspaceId, now);
        const selected = await tx.execute({
          sql: "SELECT * FROM console_scratch_cache WHERE session_id=? AND branch_id=?",
          args: [scope.sessionId, scope.branchId],
        });
        const raw = selected.rows[0];
        if (!raw) return { status: "cold" };
        if (String(raw.device_id) !== this.#deviceId) {
          await tx.execute({
            sql: "DELETE FROM console_scratch_cache WHERE session_id=? AND branch_id=?",
            args: [scope.sessionId, scope.branchId],
          });
          return { status: "unavailable", reason: "device_mismatch" };
        }
        let row: EncodedScratchCacheRow;
        try {
          row = scratchCacheRow(raw);
        } catch {
          await this.#discard(tx, scope);
          return { status: "corrupt", reason: "row_malformed" };
        }
        if (!isStructurallyValidScratchRow(row, scope, workspaceId) ||
            row.encodedRowBytes !== encodedScratchRowBytes(row) ||
            row.rowIntegrityDigest !== scratchRowIntegrityDigest(row)) {
          await this.#discard(tx, scope);
          return { status: "corrupt", reason: "row_integrity" };
        }
        try {
          await this.#assertSource(tx, scope, {
            cellId: row.sourceCellId,
            eventId: row.sourceEventId,
            cursor: cursorOf(row.sourceCursor),
          }, false);
        } catch {
          await this.#discard(tx, scope);
          return { status: "corrupt", reason: "row_malformed" };
        ...
        const accessed = encodeScratchCacheRow({ ...row, accessedAt: now });
        await tx.execute({
          sql: `UPDATE console_scratch_cache
            SET accessed_at=?,encoded_row_bytes=?,row_integrity_digest=?
            WHERE session_id=? AND branch_id=?`,
          args: [
            accessed.accessedAt,
            accessed.encodedRowBytes,
            accessed.rowIntegrityDigest,
            scope.sessionId,
            scope.branchId,
          ],
        });
        restoredSourceCursor = row.sourceCursor;
        return {
          status: "restored",
          restore: {
            candidate,
            sourceCellId: row.sourceCellId,
            checkpointedAt: row.updatedAt,
          },
        };
      }, "load scratch checkpoint");
      if (restoredSourceCursor !== null) {
        this.#rememberSourceCursor(scope, restoredSourceCursor);
      }
      return result;
    });
  }
  ...
  async write(
    ...
    assertScratchScope(scope);
    const validatedCandidate = validateScratchCheckpoint(candidate);
    const checkpoint = {
      schemaVersion: validatedCandidate.schemaVersion,
      canonicalJson: validatedCandidate.canonicalJson,
      contentDigest: validatedCandidate.digest,
      checkpointByteLength: validatedCandidate.byteLength,
      savedNamesJson: json(validatedCandidate.savedNames),
      skippedJson: json(validatedCandidate.skipped),
      empty: validatedCandidate.savedNames.length === 0 &&
        validatedCandidate.skipped.length === 0,
    } as const;
    const sourceCursor = sequenceOf(source.cursor);
    return this.#writes.run(async () => {
      const result = await this.#transaction(async (tx) => {
        const workspaceId = await this.#assertScopeFence(tx, scope, fence);
        await this.#assertSource(tx, scope, source);
        ...
        if (current) {
          try {
            await this.#assertSource(tx, scope, {
              ...
              eventId: current.sourceEventId,
              cursor: cursorOf(current.sourceCursor),
            }, false);
          } catch {
            await this.#discard(tx, scope);
            current = null;
        ...
        const row = encodeScratchCacheRow({
          sessionId: scope.sessionId,
          branchId: scope.branchId,
          workspaceId,
          deviceId: this.#deviceId,
          schemaVersion: checkpoint.schemaVersion,
          checkpointJson: checkpoint.canonicalJson,
          contentDigest: checkpoint.contentDigest,
          checkpointByteLength: checkpoint.checkpointByteLength,
          sourceCellId: source.cellId,
          sourceEventId: source.eventId,
          sourceCursor,
          savedNamesJson: checkpoint.savedNamesJson,
          skippedJson: checkpoint.skippedJson,
          createdAt,
          updatedAt: now,
          accessedAt: now,
          expiresAt: new Date(nowDate.getTime() + this.#ttlMs).toISOString(),
        });
        if (row.encodedRowBytes > this.#maxWorkspaceBytes) {
  ...
  async clear(
    ...
    return this.#writes.run(async () => {
      const result = await this.#transaction(async (tx) => {
        await this.#assertScopeFence(tx, scope, fence);
        await this.#assertSource(tx, scope, source);
        const selected = await tx.execute({
          sql: "SELECT source_cursor FROM console_scratch_cache WHERE session_id=? AND branch_id=?",
          args: [scope.sessionId, scope.branchId],
        });
        if (Math.max(
          selected.rows[0] ? Number(selected.rows[0].source_cursor) : 0,
          this.#latestSourceCursors.get(scratchScopeKey(scope)) ?? 0,
        ) >= sourceCursor) {
          return { status: "stale" } as const;
        }
        await this.#discard(tx, scope);
        return { status: "cleared" } as const;
      }, "clear scratch checkpoint");
      if (result.status === "cleared") this.#rememberSourceCursor(scope, sourceCursor);
      return result;
    });
  }

  async prune(fence: ProcessExecutionWriteFence): Promise<number> {
    return this.#writes.run(() => this.#transaction(async (tx) => {
      const workspaceId = await this.#assertWorkspaceFence(tx, fence);
      return this.#pruneExpired(tx, workspaceId, this.#timestamp());
    }, "prune scratch checkpoints"));
  }
  ...
  async #transaction<T>(
    ...
    for (let attempt = 1; ; attempt++) {
      let tx: Transaction | undefined;
      try {
        await this.#client.execute(`PRAGMA busy_timeout=${SQLITE_BUSY_TIMEOUT_MS}`);
        ...
        if (!isSqliteContention(error)) throw error;
        if (attempt >= SQLITE_CONTENTION_ATTEMPTS) {
          throw new DependencyFailureError(`SQLite contention prevented ${label}`, {
            reason: "sqlite_contention_exhausted",
            attempts: attempt,
            sqliteCode: sqliteErrorCode(error),
          });
        ...
        const ceiling = Math.min(
          SQLITE_RETRY_CAP_MS,
          SQLITE_RETRY_BASE_MS * (2 ** (attempt - 1)),
        );
        await Bun.sleep(Math.floor(Math.random() * (ceiling + 1)));
      } finally {
        try { tx?.close(); } catch { /* already committed or rolled back */ }
      }
    }
  }

  async #assertScopeFence(
    tx: Transaction,
    scope: ScratchScope,
    fence: ProcessExecutionWriteFence,
  ): Promise<string> {
    const selected = await tx.execute({
      sql: `SELECT s.workspace_id,s.root_session_id,b.branch_id
        FROM sessions s JOIN branches b ON b.session_id=s.session_id
        WHERE s.session_id=? AND b.branch_id=?`,
      args: [scope.sessionId, scope.branchId],
    });
    const row = selected.rows[0];
    if (!row) throw new NotFoundError("session branch", `${scope.sessionId}/${scope.branchId}`);
    const workspaceId = String(row.workspace_id);
    await this.#assertWorkspaceFence(tx, fence, workspaceId);
    const rootSessionId = String(row.root_session_id ?? scope.sessionId);
    if (!fence.root || fence.root.scope.kind !== "root" ||
        fence.root.scope.rootSessionId !== rootSessionId) {
      throw new ExecutionOwnershipConflictError(
        "Scratch cache mutation requires the matching root execution fence",
        { reason: "root_fence_missing_or_mismatched", rootSessionId },
      );
    }
    const root = await assertScratchLeaseProof(tx, fence.root);
    if (String(root.workspace_id) !== workspaceId ||
        String(root.owner_device_id) !== this.#deviceId ||
        root.owner_device_id !== fence.workspace.ownerDeviceId ||
        root.owner_process_id !== fence.workspace.ownerProcessId) {
      throw new ExecutionOwnershipConflictError(
        "Scratch cache fences do not share one current device owner",
        { reason: "nested_fence_owner_mismatch", rootSessionId },
      );
    }
    return workspaceId;
  }

  async #assertWorkspaceFence(
    tx: Transaction,
    fence: ProcessExecutionWriteFence,
    expectedWorkspaceId?: string,
  ): Promise<string> {
    if (fence.workspace.scope.kind !== "workspace") {
      throw new ValidationError("Scratch cache workspace fence must use workspace scope");
    }
    const workspace = await assertScratchLeaseProof(tx, fence.workspace);
    const workspaceId = fence.workspace.scope.workspaceId;
    if (String(workspace.workspace_id) !== workspaceId ||
        (expectedWorkspaceId !== undefined && workspaceId !== expectedWorkspaceId)) {
      throw new ExecutionOwnershipConflictError(
        "Scratch cache fence targets another workspace",
        { reason: "workspace_mismatch", workspaceId },
      );
    }
    if (fence.workspace.ownerDeviceId !== this.#deviceId) {
      throw new ExecutionOwnershipConflictError(
        "Scratch cache fence belongs to another device",
  ...
  async #assertSource(
    ...
    if (!source.cellId.trim() || !source.eventId.trim()) {
      throw new ValidationError("Scratch checkpoint source identity is required");
    }
    const sequence = sequenceOf(source.cursor);
    const selected = await tx.execute({
      sql: `SELECT id,sequence,session_id,branch_id,type,payload_json,origin_device_id
        FROM events WHERE id=?`,
      args: [source.eventId],
    });
    const event = selected.rows[0];
    if (!event || String(event.type) !== "CellCommitted" ||
        String(event.session_id) !== scope.sessionId ||
        String(event.branch_id) !== scope.branchId ||
        Number(event.sequence) !== sequence ||
        String(event.origin_device_id ?? "") !== this.#deviceId) {
      throw new ValidationError("Scratch checkpoint source is not the exact current-device committed cell event");
    }
    let payload: { cellId?: unknown };
    try { payload = JSON.parse(String(event.payload_json)); }
    catch { throw new ValidationError("Scratch checkpoint source event payload is malformed"); }
    if (payload.cellId !== source.cellId) {
      throw new ValidationError("Scratch checkpoint source cell does not match its event");
    }
    if (!requireLatest) return;
    const latest = await tx.execute({
      sql: `SELECT id,sequence FROM events
        WHERE session_id=? AND branch_id=? AND type='CellCommitted'
        ORDER BY sequence DESC LIMIT 1`,
      args: [scope.sessionId, scope.branchId],
    });
    if (!latest.rows[0] || String(latest.rows[0].id) !== source.eventId ||
        Number(latest.rows[0].sequence) !== sequence) {
      throw new ValidationError("Scratch checkpoint source is not the latest direct-branch committed cell");
    }
  }
  ...
  async #pruneExpired(
    ...
    const removed = await tx.execute({
      sql: "DELETE FROM console_scratch_cache WHERE workspace_id=? AND expires_at<=?",
      args: [workspaceId, now],
    });
    return removed.rowsAffected;
  }

  async #evictForUpsert(
    tx: Transaction,
    row: EncodedScratchCacheRow,
  ): Promise<void> {
    const selected = await tx.execute({
      sql: `SELECT * FROM console_scratch_cache
        WHERE workspace_id=? AND NOT(session_id=? AND branch_id=?)
        ORDER BY accessed_at,session_id,branch_id`,
      args: [row.workspaceId, row.sessionId, row.branchId],
    });
    const candidates: EncodedScratchCacheRow[] = [];
    for (const raw of selected.rows) {
      const candidate = scratchCacheRow(raw);
      const scope = {
        sessionId: candidate.sessionId,
        branchId: candidate.branchId,
      };
      let valid = candidate.deviceId === this.#deviceId &&
        isStructurallyValidScratchRow(candidate, scope, row.workspaceId) &&
        candidate.encodedRowBytes === encodedScratchRowBytes(candidate) &&
        candidate.rowIntegrityDigest === scratchRowIntegrityDigest(candidate);
      if (valid) {
        try {
          await this.#assertSource(tx, scope, {
            cellId: candidate.sourceCellId,
            eventId: candidate.sourceEventId,
            cursor: cursorOf(candidate.sourceCursor),
          }, false);
        } catch {
          valid = false;
        }
    ...
    let bytes = candidates.reduce(
      (total, candidate) => total + candidate.encodedRowBytes,
      row.encodedRowBytes,
    );
    for (const candidate of candidates) {
      if (count <= this.#maxBranches && bytes <= this.#maxWorkspaceBytes) break;
      await this.#discard(tx, candidate);
      count--;
      bytes -= candidate.encodedRowBytes;
    }
    if (count > this.#maxBranches || bytes > this.#maxWorkspaceBytes) {
      throw new ValidationError("Scratch checkpoint cannot fit within workspace cache quotas");
    }
  }

  async #discard(tx: Transaction, scope: ScratchScope): Promise<void> {
    await tx.execute({
      sql: "DELETE FROM console_scratch_cache WHERE session_id=? AND branch_id=?",
      args: [scope.sessionId, scope.branchId],
    });
  ...
  #date(): Date {
    ...
    if (!Number.isFinite(value.getTime())) throw new ValidationError("Scratch cache clock is invalid");
    return value;
  }
  ...
  #rememberSourceCursor(scope: ScratchScope, cursor: number): void {
    const key = scratchScopeKey(scope);
    const previous = this.#latestSourceCursors.get(key) ?? 0;
    this.#latestSourceCursors.delete(key);
    this.#latestSourceCursors.set(
      key,
      Math.max(previous, cursor),
    );
    while (this.#latestSourceCursors.size > this.#maxBranches * 2) {
      const oldest = this.#latestSourceCursors.keys().next().value as string | undefined;
      if (oldest === undefined) break;
      this.#latestSourceCursors.delete(oldest);
    }
  }
}

async function assertScratchLeaseProof(
  tx: Transaction,
  proof: ProcessExecutionLeaseProof,
): Promise<Row> {
  const parts = leaseScopeParts(proof.scope);
  assertLeaseOwner(proof.ownerDeviceId, proof.ownerProcessId);
  assertFenceToken(proof.fenceToken);
  ...
  if (!selected.rows[0]) {
    throw new ExecutionOwnershipConflictError(
      "Scratch cache write was rejected because its execution fence is stale",
      { reason: "stale_fence_owner_or_expiry", scopeKind: parts.scopeKind, scopeId: parts.scopeId },
    );
  }
  return selected.rows[0];
}

function assertExactFileUrl(value: string, label: string): void {
  let url: URL;
  try { url = new URL(value); }
  catch { throw new ValidationError(`${label} requires a valid file: workspace database URL`); }
  if (url.protocol !== "file:" || url.username || url.password || url.hostname ||
      url.search || url.hash || !url.pathname ||
      url.pathname === ":memory:" || url.pathname === "/:memory:") {
    throw new ValidationError(`${label} requires an exact file: workspace database URL`);
  }
}

function positiveScratchLimit(
  ...
  if (!Number.isSafeInteger(selected) || selected < 1) {
    throw new ValidationError(`Scratch cache ${label} must be a positive safe integer`);
  }
  return selected;
}

function assertScratchScope(scope: ScratchScope): void {
  if (!scope.sessionId.trim() || !scope.branchId.trim()) {
    throw new ValidationError("Scratch cache requires an exact session and branch");
  }
}

function scratchScopeKey(scope: ScratchScope): string {
  return `${scope.sessionId.length}:${scope.sessionId}${scope.branchId}`;
}

function scratchCacheRow(row: Row): EncodedScratchCacheRow {
  return {
    ...
    checkpointJson: String(row.checkpoint_json),
    contentDigest: String(row.content_digest),
    rowIntegrityDigest: String(row.row_integrity_digest),
    checkpointByteLength: Number(row.checkpoint_byte_length),
    encodedRowBytes: Number(row.encoded_row_bytes),
    sourceCellId: String(row.source_cell_id),
    sourceEventId: String(row.source_event_id),
    sourceCursor: Number(row.source_cursor),
    savedNamesJson: String(row.saved_names_json),
    skippedJson: String(row.skipped_json),
    createdAt: String(row.created_at),
...
function isStructurallyValidScratchRow(
  ...
  return row.sessionId === scope.sessionId &&
    ...
    /^[a-f0-9]{64}$/.test(row.rowIntegrityDigest) &&
    Number.isSafeInteger(row.checkpointByteLength) &&
    row.checkpointByteLength >= 0 &&
    Number.isSafeInteger(row.encodedRowBytes) &&
    row.encodedRowBytes >= 0 &&
    row.sourceCellId.length > 0 &&
    row.sourceEventId.length > 0 &&
    Number.isSafeInteger(row.sourceCursor) &&
    row.sourceCursor >= 1 &&
    isCanonicalScratchTimestamp(row.createdAt) &&
    isCanonicalScratchTimestamp(row.updatedAt) &&
    isCanonicalScratchTimestamp(row.accessedAt) &&
    isCanonicalScratchTimestamp(row.expiresAt);
}

function isCanonicalScratchTimestamp(value: string): boolean {
  const parsed = new Date(value);
  return Number.isFinite(parsed.getTime()) && parsed.toISOString() === value;
}

function encodeScratchCacheRow(input: ScratchCacheRowData): EncodedScratchCacheRow {
  let encodedRowBytes = 0;
  for (let attempt = 0; attempt < 8; attempt++) {
    const rowIntegrityDigest = scratchRowIntegrityDigest({ ...input, encodedRowBytes });
    const candidate = { ...input, encodedRowBytes, rowIntegrityDigest };
    const nextBytes = encodedScratchRowBytes(candidate);
    if (nextBytes === encodedRowBytes) {
      return candidate;
    }
    encodedRowBytes = nextBytes;
  }
  throw new ValidationError("Scratch cache row byte accounting did not converge");
}

function scratchRowIntegrityDigest(
  row: ScratchCacheRowData & { readonly encodedRowBytes: number },
): string {
  const input = {
    ...
    checkpointByteLength: row.checkpointByteLength,
    encodedRowBytes: row.encodedRowBytes,
    sourceCellId: row.sourceCellId,
    sourceEventId: row.sourceEventId,
    sourceCursor: row.sourceCursor,
    savedNamesJson: row.savedNamesJson,
  ...
  return sha256(canonicalScratchJson(input));
}

function encodedScratchRowBytes(row: EncodedScratchCacheRow): number {
  return new TextEncoder().encode(canonicalScratchJson(row)).byteLength;
}

function canonicalScratchJson(value: unknown): string {
  if (value === null || typeof value !== "object") return JSON.stringify(value);
  if (Array.isArray(value)) return `[${value.map(canonicalScratchJson).join(",")}]`;
  const record = value as Record<string, unknown>;
  return `{${Object.keys(record).sort().map((key) =>
    `${JSON.stringify(key)}:${canonicalScratchJson(record[key])}`).join(",")}}`;
}
...
const privateTables = /\b(schema_migrations|outbox|snapshots|process_execution_leases|console_scratch_cache|device_clocks|workspace_replica_status|sync_ingest_receipts|sync_origin_watermarks|sync_quarantine|sync_branch_mappings|sync_reconciliations|data_manifests|sqlite_(?:schema|master|temp_schema|temp_master|sequence))\b/i;
