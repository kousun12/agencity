...
export class ProfileStore implements ProfileDatabase {
  ...
  async stageGlobalSkill(input:StageGlobalSkillInput):Promise<ProfileGlobalSkillRecord>{
    ...
    const tx=await this.#client.transaction("write");
    try{
      const prior=(await tx.execute({sql:"SELECT rowid AS action_sequence,* FROM profile_skill_actions WHERE idempotency_key=?",args:[idempotencyKey]})).rows[0];
      if(prior){action=rowToProfileGlobalSkillAction(prior);if(action.requestDigest!==requestDigest)throw new ConflictError("Global skill action idempotency key was reused with different meaning",{idempotencyKey});}
        ...
        const inserted=await tx.execute({sql:"INSERT INTO profile_skill_actions(action_id,skill_id,version_id,digest,action,previous_availability,availability,effect_ref,idempotency_key,request_digest,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)",args:[actionId,input.skillId,versionId,digest,"staged",current?.availability??null,availability,effectRef,idempotencyKey,requestDigest,now]});
        action={sequence:Number(inserted.lastInsertRowid),actionId,skillId:input.skillId,versionId,digest,action:"staged",previousAvailability:current?.availability??null,availability,effectRef,idempotencyKey,requestDigest,createdAt:now};
      }
      await tx.commit();
    }catch(error){if(!tx.closed)await tx.rollback();throw error;}finally{tx.close();}
  ...
  async #setGlobalSkillAvailability(input:SetGlobalSkillAvailabilityInput):Promise<ProfileGlobalSkillRecord>{return this.#writes.run(()=>this.#setGlobalSkillAvailabilityUnlocked(input));}
  async #setGlobalSkillAvailabilityUnlocked(input:SetGlobalSkillAvailabilityInput):Promise<ProfileGlobalSkillRecord>{
    assertProfileSkillId(input.skillId,"Global skill ID");assertProfileSkillId(input.expectedVersionId,"Expected global skill version ID");assertProfileSkillDigest(input.expectedDigest,"Expected global skill digest");assertProfileSkillAvailability(input.expectedAvailability);if(!Number.isSafeInteger(input.expectedActionSequence)||input.expectedActionSequence<1)throw new ValidationError("Expected global skill action sequence is invalid");assertProfileSkillId(input.idempotencyKey,"Global skill idempotency key");const effectRef=input.effectRef??null;assertProfileSkillEffectRef(effectRef);
    const requestDigest=profileSkillRequestDigest({action:"status-changed",skillId:input.skillId,availability:input.availability,expectedVersionId:input.expectedVersionId,expectedDigest:input.expectedDigest,expectedAvailability:input.expectedAvailability,expectedActionSequence:input.expectedActionSequence,effectRef}),now=new Date().toISOString();let action!:ProfileGlobalSkillAction;
    const tx=await this.#client.transaction("write");
    try{
      const prior=(await tx.execute({sql:"SELECT rowid AS action_sequence,* FROM profile_skill_actions WHERE idempotency_key=?",args:[input.idempotencyKey]})).rows[0];
      if(prior){action=rowToProfileGlobalSkillAction(prior);if(action.requestDigest!==requestDigest)throw new ConflictError("Global skill action idempotency key was reused with different meaning",{idempotencyKey:input.idempotencyKey});}
        ...
        const latestRow=(await tx.execute({sql:"SELECT rowid AS action_sequence,* FROM profile_skill_actions WHERE skill_id=? ORDER BY rowid DESC LIMIT 1",args:[input.skillId]})).rows[0];if(!latestRow)throw new ConflictError("Global skill action history is missing",{skillId:input.skillId});const latest=rowToProfileGlobalSkillAction(latestRow);
        if(current.versionId!==input.expectedVersionId||current.digest!==input.expectedDigest||current.availability!==input.expectedAvailability||latest.sequence!==input.expectedActionSequence||latest.availability!==input.expectedAvailability)throw new ConflictError("Global skill status compare-and-swap failed",{skillId:input.skillId});assertProfileSkillTransition(current.availability,input.availability,current);
        const changed=await tx.execute({sql:"UPDATE profile_skills SET availability=?,updated_at=? WHERE skill_id=? AND current_version_id=? AND availability=?",args:[input.availability,now,input.skillId,input.expectedVersionId,input.expectedAvailability]});if(changed.rowsAffected!==1)throw new ConflictError("Global skill status compare-and-swap failed",{skillId:input.skillId});
        const actionId=newId();
        const inserted=await tx.execute({sql:"INSERT INTO profile_skill_actions(action_id,skill_id,version_id,digest,action,previous_availability,availability,effect_ref,idempotency_key,request_digest,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)",args:[actionId,current.skillId,current.versionId,current.digest,"status-changed",current.availability,input.availability,effectRef,input.idempotencyKey,requestDigest,now]});
        action={sequence:Number(inserted.lastInsertRowid),actionId,skillId:input.skillId,versionId:current.versionId,digest:current.digest,action:"status-changed",previousAvailability:current.availability,availability:input.availability,effectRef,idempotencyKey:input.idempotencyKey,requestDigest,createdAt:now};
      }
      await tx.commit();
    }catch(error){if(!tx.closed)await tx.rollback();throw error;}finally{tx.close();}
    return this.#profileSkillActionSnapshot(action);
