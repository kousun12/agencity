  ...
  async handle(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (parts[0] === "sessions" && parts[1]) {
        ...
        if (parts[2] === "mailbox" && parts[3] && parts[4] === "result" && branchId && request.method === "GET") return Response.json(await this.supervisor.agents.messageResult(sessionId, branchId, decodeURIComponent(parts[3]), { wait: false }));
