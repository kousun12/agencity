...
const SDK_GUIDE = [
  ...
  "Cell globals: sdk, sql, session, console, state, artifacts, tools, inspect, cells, ai.",
  ...
  "tools.shell(command, options?) returns agencity.bounded-output.v1. Check result.completeness before reading fields: inline has result.value with { exitCode, stdout, stderr }; spilled and truncated have result.preview, and only spilled has result.artifact. Return the original envelope or an ordinary object; never copy protocol/completeness onto a partial wrapper.",
  ...
  "Use artifacts.put(content: string, mediaType?: string) for durable content. Use artifacts.readRange(artifactId, start, end) only with 0 <= start <= end <= the artifact's known immutable size and end - start <= 64 KiB; decode or parse an inline result's value.bytes inside the cell.",
  ...
  "Raw ai calls cannot inspect files, run commands, use skills, call tools, read ambient branch messages, profiles, memory, or repository instructions, or continue autonomously. Use sdk.agents.run when a strictly narrower child task must inspect the workspace, use tools, run commands, or iterate before this cell continues. Every run, runMany, spawn, or spawnMany child input must include a concise stable human-readable name. When the task establishes a named identity such as Atlas, use that exact declared name; keep the task separately descriptive. Do not hand off your entire assigned task, restate it as a child task, or recurse solely because the work is agentic; continue directly unless the child has a bounded independent outcome that you will use. Give agents.run an output schema when its conclusion must be program data; omit output when a textual report is enough.",
  "Use sdk.agents.spawn when a child should work independently and its result is not needed before this cell continues. Retain the handle and use messaging, terminal notices, handle.result(options), or sdk.agents.result(handle, options) later. Handle.result is a worker-local convenience; the durable JSON handle remains the task/run identity. runMany and spawnMany each accept the input array directly, never an object such as { tasks: [...] }, and each returns its result array directly, never a { results: [...] } or { items: [...] } wrapper. Use runMany or spawnMany only for bounded independent tasks, never for dependent steps. All generation and agent operations use the same canonical creator/model IDs and caller-narrowing model policy.",
  "Cross-agent callable behavior uses retained messaging and artifacts; durable versioned RPC is available only when a separately advertised capability implements it. Model-generated text and object judgments are data, not objective evidence, factual proof, completion proof, or expanded runtime authority. A long parent-cell loop is not a durable coordinator across worker loss; use durable agent handles, messages, tasks, state, or artifacts at recovery boundaries.",
  "Raw text example: const summary = await ai.generateText({ prompt: \"Summarize these failures without inventing causes.\", context: failures });",
  ...
  "Awaited agent example: const { z } = await import(\"zod\"); const review = await sdk.agents.run({ name: \"Readiness Reviewer\", task: \"Inspect the implementation, run relevant tests, and assess readiness.\", output: { schema: z.object({ ready: z.boolean(), evidence: z.array(z.string()), remainingWork: z.array(z.string()) }) } }); if (review.status === \"succeeded\" && review.output.kind === \"object\" && !review.output.object.ready) return review.output.object.remainingWork;",
  "Detached agent example: const audit = await sdk.agents.spawn({ name: \"Compatibility Auditor\", task: \"Run the slow compatibility audit and report back to the parent.\" }); return { auditTaskId: audit.taskId };",
