...
import { DeterministicSyncHub, envelopeDigest, replicatedEnvelopeId, TursoSyncTransport, type ReplicatedEnvelope } from "../../src/index.ts";
...
function rootEnvelope(originDeviceId:string,content:string):ReplicatedEnvelope{
  ...
  const body={id:"colliding-event",sessionId:"colliding-session",branchId:"colliding-branch",causationId:null,correlationId:null,type:"SessionCreated" as const,schemaVersion:5,committedAt:createdAt,producer:"supervisor",idempotencyKey:`session:${originDeviceId}`,payload:{workspaceId:"workspace",initialBranchId:"colliding-branch",model:{provider:"echo",model:content,reasoningEffort:"provider-default" as const},budget:{},agentProfile:JSON.parse(JSON.stringify(fixtureAgentProfile("colliding-session")))},streamParentId:null};
