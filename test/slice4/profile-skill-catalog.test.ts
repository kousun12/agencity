...
describe("profile global skill catalog storage",()=>{
 ...
 test("uses CAS lifecycle actions, hides unavailable pointers by default, and retains removed history",async()=>{
  ...
  const disabled=await store.setGlobalSkillStatus({skillId:staged.skillId,status:"disabled",expectedVersionId:staged.versionId,expectedDigest:staged.digest,idempotencyKey:"status:lifecycle:disable",effectRef:"effect:disable:1"});
  expect(disabled.status).toBe("disabled");expect(await store.getGlobalSkill(staged.skillId)).toBeNull();expect(await store.listGlobalSkillCatalog()).toEqual([]);expect((await store.listGlobalSkillStatuses({includeUnavailable:true}))[0]?.availability).toBe("disabled");
  const retry=await store.setGlobalSkillStatus({skillId:staged.skillId,status:"disabled",expectedVersionId:staged.versionId,expectedDigest:staged.digest,idempotencyKey:"status:lifecycle:disable",effectRef:"effect:disable:1"});expect(retry).toEqual(disabled);
  await expect(store.setGlobalSkillStatus({skillId:staged.skillId,status:"removed",expectedVersionId:staged.versionId,expectedDigest:staged.digest,idempotencyKey:"status:lifecycle:disable",effectRef:"effect:disable:1"})).rejects.toBeInstanceOf(ConflictError);
  await expect(store.setGlobalSkillStatus({skillId:staged.skillId,status:"enabled",expectedVersionId:"version:stale",expectedDigest:staged.digest,idempotencyKey:"status:lifecycle:stale"})).rejects.toBeInstanceOf(ConflictError);
  const enabled=await store.setGlobalSkillAvailability({skillId:staged.skillId,availability:"enabled",expectedVersionId:staged.versionId,expectedDigest:staged.digest,idempotencyKey:"status:lifecycle:enable"});expect(enabled.versionId).toBe(staged.versionId);expect((await store.getGlobalSkill(staged.skillId))?.digest).toBe(staged.digest);
  const removed=await store.setGlobalSkillStatus({skillId:staged.skillId,status:"removed",expectedVersionId:staged.versionId,expectedDigest:staged.digest,idempotencyKey:"status:lifecycle:remove"});expect(removed.availability).toBe("removed");expect(await store.getGlobalSkill(staged.skillId)).toBeNull();expect(await store.getGlobalSkillVersion(staged.versionId)).not.toBeNull();
  await expect(store.setGlobalSkillStatus({skillId:staged.skillId,status:"enabled",expectedVersionId:staged.versionId,expectedDigest:staged.digest,idempotencyKey:"status:lifecycle:restore"})).rejects.toBeInstanceOf(ConflictError);
  const history=await store.listGlobalSkillHistory(staged.skillId);expect(history?.versions).toHaveLength(1);expect(history?.actions.map(item=>item.availability)).toEqual(["enabled","disabled","enabled","removed"]);
