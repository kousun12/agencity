...
describe("terminal inspector view models", () => {
  test.each([
    ...
    ["/tree", { family: { items: [{ name: "reviewer", relationship: "child", status: "idle", taskStatus: "completed", task: "Review the patch", cancellationRequested: false, activity: "attention", activityReason: "unknown" }] }, tasks: [{ task: "Review", status: "completed", model: { provider: "openai", model: "gpt-test" }, result: "Done" }], mailbox: { items: [] } }, "unknown outcome"],
