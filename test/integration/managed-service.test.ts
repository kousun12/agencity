...
describe("managed workspace service", () => {
  ...
  test("elects one service, publishes owner-only discovery, and requires bearer auth on every route", async () => {
    ...
    expect(health).toMatchObject({ authenticated: true, workspaceId: config.workspace.workspaceId, instanceId: service.manifest.instanceId, appVersion: "0.1.0-test", protocolMin: 3, protocolMax: 3, configHash: managedServiceConfigurationHash(config) });
